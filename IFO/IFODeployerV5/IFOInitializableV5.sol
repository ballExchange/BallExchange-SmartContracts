...
contract IFOInitializableV5 is IIFOV5, ReentrancyGuard, Whitelist {
    ...
    struct PoolCharacteristics {
        ...
        bool maxTotalAmountPool; // tax on the overflow (if any, it works with _calculateTaxOverflow)
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        if(_poolInformation[_pid].maxTotalAmountPool){
            if(_poolInformation[_pid].totalAmountPool.add(_amount) > _poolInformation[_pid].raisingAmountPool){
                _amount = _poolInformation[_pid].raisingAmountPool - _poolInformation[_pid].totalAmountPool;
                require(_amount > 0, 'Deposit: Max raising amount pool');
            }
        }
     ...
     * @param _maxTotalAmountPool: slice period seconds of vesting
    ...
    function setPool(
        ...
        _poolInformation[_pid].maxTotalAmountPool = _maxTotalAmountPool;
    ...
    function viewPoolInformation(
        uint256 _pid
    ) external view override returns (uint256, uint256, uint256, bool, uint256, uint256, bool) {
        return (
