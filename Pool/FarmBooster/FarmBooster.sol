...
contract FarmBooster is Ownable {
    ...
    function deactive(uint256 _pid) external {
        ...
        if (itmap.data[_pid] > BOOST_PRECISION) {
            IMasterChef(MASTER_CHEF).updateBoostMultiplier(proxy, _pid, BOOST_PRECISION);
        }
        itmap.remove(_pid);
    ...
    function _updateBoostMultiplier(
        ...
        if (!whiteList[_pid]) {
            if (itmap.data[_pid] > BOOST_PRECISION) {
                ...
                IMasterChef(MASTER_CHEF).updateBoostMultiplier(_proxy, _pid, BOOST_PRECISION);
            }
            itmap.remove(_pid);
            return;
        }

        uint256 prevMultiplier = IMasterChef(MASTER_CHEF).getBoostMultiplier(_proxy, _pid);
        ...
        if (multiplier != prevMultiplier) {
            IMasterChef(MASTER_CHEF).updateBoostMultiplier(_proxy, _pid, multiplier);
        }
    ...
    function _boostCalculate(
        ...
        (uint256 lpBalance, , ) = IMasterChef(MASTER_CHEF).userInfo(_pid, _proxy);
        ...
        IBEP20 lp = IBEP20(IMasterChef(MASTER_CHEF).lpToken(_pid));
