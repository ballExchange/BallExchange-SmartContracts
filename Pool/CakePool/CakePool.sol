...
contract CakePool is Ownable, Pausable {
     ...
     * @param _pid: cake pool ID in masterchef
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchef.deposit(cakePoolPID, balance);
        emit Init();
    }
    ...
    function harvest() internal {
        uint256 pendingCake = masterchef.pendingCake(cakePoolPID, address(this));
        if (pendingCake > 0) {
            uint256 balBefore = available();
            masterchef.withdraw(cakePoolPID, 0);
    ...
    function calculateTotalPendingCakeRewards() public view returns (uint256) {
        uint256 amount = masterchef.pendingCake(cakePoolPID, address(this));
        return amount;
    }
